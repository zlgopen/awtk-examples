...
static ret_t on_music_play_pause(void* ctx, event_t* e) {
  ...
  if (global_data.music_state.play_state == 0) {
    widget_use_style(button, "music_pause");
...
static char* play_mode[3] = {"music_circle_all", "music_circle_one", "music_rand"};
...
static void init_music_state(widget_t* win) {
  ...
  if (widget != NULL) {
    if (music_state->play_state == 0) {
      ...
      widget_use_style(widget, "music_pause");
    }
  }
  ...
  if (widget != NULL) {
    switch (music_state->play_mode) {
      case 0:
        widget_use_style(widget, "music_circle_all");
        break;
      case 1:
        widget_use_style(widget, "music_circle_one");
        break;
      case 2:

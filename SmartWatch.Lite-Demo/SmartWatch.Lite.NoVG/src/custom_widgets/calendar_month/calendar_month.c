...
static ret_t calendar_month_darw_one_month(calendar_month_t *calendar_month,
  ...
  int32_t clip_right = c->lcd->dirty_rect.x;
  int32_t clip_left = c->lcd->dirty_rect.x + c->lcd->dirty_rect.w - 1;
  int32_t clip_top = c->lcd->dirty_rect.y;
  int32_t clip_bottom = c->lcd->dirty_rect.y + c->lcd->dirty_rect.h - 1;

  if (clip_left <= clip_right || clip_bottom <= clip_top) {
    return RET_OK;
  }
  ...
  for (x = 0; x < WEEK_DAY_NUMBER; x++) {
    ...
    r.y = h * y;
#ifdef FRAGMENT_FRAME_BUFFER_SIZE
    if (r.x + c->ox > clip_left || r.x + c->ox + r.w - 1 < clip_right ||
        r.y + c->oy > clip_bottom || r.y + c->oy + r.h - 1 < clip_top) {
      continue;
    }
#endif
    if (x == 0 || x == 6) {

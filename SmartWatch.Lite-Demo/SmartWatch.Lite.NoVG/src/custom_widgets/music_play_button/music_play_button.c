...
static ret_t music_play_button_on_paint_self(widget_t *widget, canvas_t *c) {
  ...
  return_value_if_fail(music_play_button->max_value > 0, RET_BAD_PARAMS);
  if (music_play_button->line_color.rgba.a > 0x8) {
    xy_t cx = widget->w / 2;
    xy_t cy = widget->h / 2;
    ...
    if (vg != NULL) {
      ...
      float_t end_angle = 0;
      float_t start_angle = (float_t)music_play_button->start_angle;
      float_t angle = (float_t)(360 * music_play_button->value) /
                      (float_t)music_play_button->max_value;
      ...
      if (music_play_button->draw_circle_ctx == NULL) {
        music_play_button->draw_circle_ctx =
            draw_circle_fill_create((int32_t)r, music_play_button->line_width);
      }

      draw_circle_fill_arc_width(music_play_button->draw_circle_ctx, c, cx, cy,
                                 (int32_t)start_angle, (int32_t)end_angle, ccw,
                                 &(music_play_button->line_color.rgba));
    }
  }
...
static ret_t music_play_button_on_destroy(widget_t *widget) {
  music_play_button_t *music_play_button = MUSIC_PLAY_BUTTON(widget);
  return_value_if_fail(widget != NULL && music_play_button != NULL,
                       RET_BAD_PARAMS);

  if (music_play_button->draw_circle_ctx != NULL) {
    draw_circle_fill_destroy(music_play_button->draw_circle_ctx);
    music_play_button->draw_circle_ctx = NULL;
  }
...
TK_DECL_VTABLE(music_play_button) = {
    ...
    .on_destroy = music_play_button_on_destroy,

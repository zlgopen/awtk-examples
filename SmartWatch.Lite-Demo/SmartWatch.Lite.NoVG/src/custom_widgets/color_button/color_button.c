...
static ret_t color_button_on_paint_background(widget_t *widget, canvas_t *c) {
  ...
  const char *fg_image_name = style_get_str(style, STYLE_ID_FG_IMAGE, NULL);
  const char *bg_image_name = style_get_str(style, STYLE_ID_BG_IMAGE, NULL);
  ...
  if (color.rgba.a) {
    if (vg != NULL && bg_image_name == NULL) {
      ...
      if (widget_load_image(widget, bg_image_name, &bg_img) == RET_OK) {
        int32_t r1_x = (widget->w - bg_img.w) / 2;
        rect_t r1 = rect_init(r1_x, 0, bg_img.w, bg_img.h);
        if (color_button->is_orientation_top) {
          r1.y = 0;
        } else {
          r1.y = widget->h - color_button->height;
        }
        canvas_draw_image_ex(c, &bg_img, IMAGE_DRAW_CENTER, &r1);
      }
    }
  ...
  if (fg_image_name != NULL) {
    if (widget_load_image(widget, fg_image_name, &fg_img) == RET_OK) {
      r.y += color_button->yoffset;
      canvas_draw_image_ex(c, &fg_img, IMAGE_DRAW_ICON, &r);
    }
  }

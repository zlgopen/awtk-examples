...
static int32_t weather_main_update_temp_str(widget_t *label, int32_t min,
                                            int32_t max, const char *last_str) {
  char str_tmp[20];
  int32_t new_tmp = 0;
  bool_t is_update = rand() % 2;
  ...
  if (is_update || new_tmp < min || new_tmp > max) {
    new_tmp = min + rand() % (max - min);
    tk_snprintf(str_tmp, sizeof(str_tmp), "%d%s", new_tmp, last_str);
    widget_set_text_utf8(label, str_tmp);
  }
...
#ifndef WITHOUT_ANIMATOR
  weather_main_update_temp(handle->windy_label, WINDY_MIN, WINDY_MAX);
  weather_main_update_temp(handle->ultraviolet_rays_label, ULTRAVIOLET_RAY_MIN,
                           ULTRAVIOLET_RAY_MAX);
  weather_main_update_temp(handle->somatonsensory_temp_label,
                           SOMATONSENSORY_TEMP_MIN, SOMATONSENSORY_TEMP_MAX);

  weather_main_update_temp_str(handle->humidity_label, HUMIDITY_MIN,
                               HUMIDITY_MAX, "%");
  weather_main_update_temp_str(handle->pressure_label, PRESSURE_MIN,
                               PRESSURE_MAX, "hPa");
#endif
...
#ifndef WITHOUT_ANIMATOR
  handle->windy_label = widget_lookup(load_widget, WINDY_LABEL, TRUE);
  handle->humidity_label = widget_lookup(load_widget, HUMIDITY_LABEL, TRUE);
  handle->pressure_label = widget_lookup(load_widget, PRESSURE_LABEL, TRUE);
  handle->ultraviolet_rays_label =
      widget_lookup(load_widget, ULTRAVIOLET_RAY_LABEL, TRUE);
  handle->somatonsensory_temp_label =
      widget_lookup(load_widget, SOMATONSENSORY_TEMP_LABEL, TRUE);

  return_value_if_fail(handle->windy_label != NULL, RET_FAIL);
  return_value_if_fail(handle->humidity_label != NULL, RET_FAIL);
  return_value_if_fail(handle->pressure_label != NULL, RET_FAIL);
  return_value_if_fail(handle->ultraviolet_rays_label != NULL, RET_FAIL);
  return_value_if_fail(handle->somatonsensory_temp_label != NULL, RET_FAIL);
#endif
